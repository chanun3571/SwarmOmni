    ...
    def Update_Speed(self, msg):
        ...
        if self.left_wheel_power==0 and self.right_wheel_power==0 and self.center_wheel_power==0:
            ...
            if abs(self.left_wheel_power)+abs(self.right_wheel_power)+abs(self.center_wheel_power)<30:
                self.center_wheel_power = float(self.center_wheel_power) * 1.1
                self.left_wheel_power = float(self.left_wheel_power) * 1.1
                self.right_wheel_power = float(self.right_wheel_power) * 1.1           
       #power command   
